...
namespace HumanResources
{
    ...
    public static class Extension_Research
    {
        ...
        public static bool CreateStuff(this ResearchProjectDef tech, ThingFilter filter, ThingDef unset = null, ThingDef standard = null)
        {
            ...
            var stuffCatDef = DefDatabase<StuffCategoryDef>.GetNamed(tag);
            if (tech.techLevel > 0)
            {
            ...
            if (unset == null) return false;
            if (standard.stuffCategories.Contains(stuffCatDef))
            {
                unset.defaultStuff = techStuff;
                Log.Message($"DEFAULT from {standard.defName} was {tag} found with {techStuff.defName}");
                return true;
            }
            return false;
        }
